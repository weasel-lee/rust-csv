...
def group_files_by_crate(files: list[FileEntry],
                         crate_roots: list[str]) -> dict[str, list[FileEntry]]:
...
def pack_batches(by_crate: dict[str, list[FileEntry]], max_files: int,
                 max_bytes: int) -> list[Batch]:
    batches: list[Batch] = []
    ...
    for crate in sorted(by_crate.keys()):
        ...
        for rel_path, size in crate_files:
            if size > max_bytes:
                if current_files:
                    current_files, current_bytes, batch_index = flush(current_files, current_bytes,
                                                                      batch_index)
            ...
            if current_files and (len(current_files) >= max_files or
        ...
        current_files, current_bytes, batch_index = flush(current_files, current_bytes, batch_index)
...
def select_weekly_batches(batches: list[Batch],
                          week_index: int) -> tuple[list[int], list[int], list[Batch]]:
    if not batches:
        return [], [], []

    raw_indices = [((week_index * ROTATION_BATCH_COUNT) + offset) % len(batches)
...
def render_task_comments(
    *,
    ...
    for batch in selected_batches:
        ...
        kib = fmt_kib(batch.bytes)
        oversize = ' (OVERSIZE)' if batch.oversize else ''
        task_lines.append(f'Crate `{batch.crate}` — Batch {batch.index:02d}/{batch.total:02d} — '
